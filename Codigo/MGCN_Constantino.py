...
def train_step(reg, bg, labels, masks, loss_criterion, optimizer):
    ...
    node_types = torch.tensor(len(set(bg.ntypes)))

    #src, dst = bg.edges()
    #edge_dists = torch.sqrt(((src - dst ** 2).sum(dim=1, keepdims=True)))
    print(bg.edata)

    src_nodes, dst_nodes = bg.in_edges(range(bg.number_of_nodes()))
    #src_nodes, dst_nodes = bg.edges()
    edge_dists = torch.sqrt(((bg.nodes[src_nodes].data['h'] - bg.nodes[dst_nodes].data['h']) ** 2).sum(dim=1, keepdim=True))

    prediction = reg(bg, node_types, edge_dists)
...
def compute_edge_dists(g):
    """
    Compute the pairwise Euclidean distance between the atom features of all pairs
    of atoms in each graph in the list of DGLGraphs g.
    """
    edge_feats = g.edata['feat']
    src_feats = edge_feats[:, :edge_feats.shape[1]//2]
    dst_feats = edge_feats[:, edge_feats.shape[1]//2:]
    dist = ((src_feats - dst_feats)**2).sum(dim=1).sqrt()
