...
if __name__ == '__main__':
    ...
    reg = GCNPredictor(in_feats=graph.ndata['h'].shape[1],
                       hidden_feats=[64, 64],
    ...
    # Initialize dropout and activation layers
    dropout_layers = [torch.nn.Dropout(p=dropout) for _ in range(num_layers)]
    reg.dropout_layers = torch.nn.ModuleList(dropout_layers)
    activation_layers = [torch.nn.ReLU() for _ in range(num_layers)]
    reg.activation_layers = torch.nn.ModuleList(activation_layers)
    ...
    losses.to_csv('losses_gcnPredictor_64hiddenFeats.csv', index=False)
    print('Done')
