...
def train_step(reg, bg, labels, masks, loss_criterion, optimizer):
    ...
    prediction = reg(bg, categorical_node_feats, categorical_edge_feats)
...
if __name__ == '__main__':
    ...
    num_layers = 5 #Valor por defecto en el modelo GIN, No se usa
    dropout = 0.2 #No se usa
    learning_rate = 1e-3
    SEED = 129767345
    #########################
    rt_scaler = 'robust'
    ...
    X, y = load_mols_df() #-> Solo coge 100 muestras para ir más rápido
    #X, y = load_mols_df()
    ...
    #reg = GINPredictor(num_node_emb_list=[len(graph.ndata['atomic_number'].unique()), len(graph.ndata['chirality_type'].unique())],
    #                   num_edge_emb_list=[len(graph.edata['bond_type'].unique()),len(graph.edata['bond_direction_type'].unique())])

    #Setting values based on observations from https://github.com/awslabs/dgl-lifesci/blob/be8bc71d29ecf34a9dab7c7bd47c08f3383d9be0/python/dgllife/model/pretrain/property_prediction.py
    reg = GINPredictor(num_node_emb_list=[120, 3],
    ...
    losses.to_csv('losses_GIN.csv', index=False)
    print('Done')
